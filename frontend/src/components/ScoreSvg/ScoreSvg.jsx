...
const ScoreSvg = ({ score = 0, className = "" }) => {
  const pathRef = useRef(null);
  const [dotPos, setDotPos] = useState({ x: 100, y: 350 });

  const s = Math.max(0, Math.min(100, score));
  const ratio = s / 100;
  const totalLen = 785.4;
  const dashOffset = totalLen * (1 - ratio);
  const isSad = s < 50;

  // 最准：用浏览器原生 getPointAtLength 取路径上的点
  useEffect(() => {
  ...
  // 开心嘴（原）
  const mouthHappy =
    "M347.167 316.482C339.696 316.482 332.608 313.623 328.204 308.835C327.391 307.953 327.449 306.58 328.331 305.768C329.213 304.956 330.588 305.013 331.399 305.896C334.996 309.807 340.89 312.141 347.167 312.141C353.443 312.141 359.338 309.807 362.935 305.896C363.745 305.013 365.121 304.956 366.003 305.768C366.885 306.58 366.942 307.953 366.13 308.835C361.725 313.623 354.637 316.482 347.167 316.482Z";

  // 悲伤嘴：整体上移 6px，你要再往上就改这里 -8 -10 都行
  const mouthSad =
    "M347.167 310.482C339.696 310.482 332.608 313.341 328.204 318.129C327.391 319.011 327.449 320.384 328.331 321.196C329.213 322.008 330.588 321.951 331.399 321.068C334.996 317.157 340.89 314.823 347.167 314.823C353.443 314.823 359.338 317.157 362.935 321.068C363.745 321.951 365.121 322.008 366.003 321.196C366.885 320.384 366.942 319.011 366.13 318.129C361.725 313.341 354.637 310.482 347.167 310.482Z";
  ...
  return (
    ...
    >
      ...
      {/* 进度末端白点：绝对精准 */}
      <circle cx={dotPos.x} cy={dotPos.y} r="12" fill="#fff" />

      {/* 蓝色小圆点 */}
      <circle cx="140" cy="350" r="5" fill="#2f49d0" />
      <circle cx="144.589" cy="306.339" r="5" fill="#2f49d0" />
      <circle cx="158.155" cy="264.585" r="5" fill="#2f49d0" />
      <circle cx="180.106" cy="226.565" r="5" fill="#2f49d0" />
      <circle cx="209.483" cy="193.940" r="5" fill="#2f49d0" />
      <circle cx="245" cy="168.135" r="5" fill="#2f49d0" />
      <circle cx="285.106" cy="150.278" r="5" fill="#2f49d0" />
      <circle cx="328.049" cy="141.150" r="5" fill="#2f49d0" />
      <circle cx="371.951" cy="141.150" r="5" fill="#2f49d0" />
      <circle cx="414.894" cy="150.278" r="5" fill="#2f49d0" />
      <circle cx="455" cy="168.135" r="5" fill="#2f49d0" />
      <circle cx="490.517" cy="193.940" r="5" fill="#2f49d0" />
      <circle cx="519.894" cy="226.565" r="5" fill="#2f49d0" />
      <circle cx="541.845" cy="264.585" r="5" fill="#2f49d0" />
      <circle cx="555.411" cy="306.339" r="5" fill="#2f49d0" />
      <circle cx="560" cy="350" r="5" fill="#2f49d0" />
      ...
      {/* 嘴巴：悲伤已上移 6px */}
      <path d={isSad ? mouthSad : mouthHappy} fill="#AA7346" />
