"""
build_law_embeddings.py
=======================
对 knowledge_graph/processed_data/law_chunks.jsonl 做 DashScope 文本向量化，
输出到 law_vectors.jsonl，支持断点续传。
"""
...
DEFAULT_INPUT = PROCESSED_DATA_DIR / "law_chunks.jsonl"
DEFAULT_OUTPUT = PROCESSED_DATA_DIR / "law_vectors.jsonl"
...
def iter_law_records(input_path: Path) -> Iterable[Dict]:
...
def build_text(record: Dict) -> str:
    ...
    if not isinstance(hierarchy, list):
        hierarchy = [str(hierarchy)] if hierarchy else []
    parts = [
        str(record.get("law_name", "")).strip(),
        str(record.get("article_no", "")).strip(),
        " > ".join(str(x).strip() for x in hierarchy if str(x).strip()),
        str(record.get("relevance", "")).strip(),
...
def embed_batch(texts: List[str], batch_idx: int) -> List[List[float]]:
    if batch_idx == 0:
        print(f"[batch {batch_idx}] 准备发起首批 law embedding 请求，文本数: {len(texts)}", flush=True)
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            ...
            if batch_idx == 0:
                print(f"[batch {batch_idx}] 首批 law embedding 请求已返回，开始解析结果", flush=True)
            ...
            print(
                f"[batch {batch_idx}] 第 {attempt} 次网络请求失败: {exc.__class__.__name__}: {exc}，{wait_seconds:.1f}s 后重试",
                flush=True,
            )
    ...
    if batch_idx == 0:
        for text_idx, text in enumerate(texts):
            if len(text) >= MAX_INPUT_LENGTH:
                print(f"[batch {batch_idx}] 第 {text_idx} 条法条文本过长，已裁剪到 {len(text)} 字符以内", flush=True)
        ...
        payloads.append(
            {
                ...
                "source_file": record.get("_source_file", "law_chunks.jsonl"),
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Build law embeddings with DashScope")
    parser.add_argument("--input", default=str(DEFAULT_INPUT))
    parser.add_argument("--output", default=str(DEFAULT_OUTPUT))
    parser.add_argument("--batch-size", type=int, default=DEFAULT_BATCH_SIZE)
    parser.add_argument("--workers", type=int, default=DEFAULT_WORKERS, help="并发线程数，建议 2~4")
    ...
    if not input_path.exists():
        raise FileNotFoundError(f"law chunks 文件不存在: {input_path}")
